...
class DirectBackend(PasswordBackend):
    ...
    @classmethod
    def create(cls, settings: DirectBackendSettings | None = None) -> "DirectBackend":
        ...
        if not password_store_dir.is_dir():
            raise BackendError(
                f"Password store directory not found: {password_store_dir}"
            )
    ...
    def get_password(self, name: str) -> PasswordEntry | None:
        ...
        try:
            ...
            username = metadata.get(
                "username", metadata.get("user", metadata.get("login", ""))
            )
